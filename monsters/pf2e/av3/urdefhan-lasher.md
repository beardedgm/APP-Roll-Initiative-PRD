# Urdefhan Lasher
*Creature 7*

ne, medium, humanoid, urdefhan

---

**Perception** +16

**Languages** aklo, daemonic, undercommon

**STR** +5, **DEX** +3, **CON** +4, **INT** +0, **WIS** +3, **CHA** +2

**Items** chain shirt, +1 striking shauth lash (page 73)

---

**AC** 24

**Fort** +17, **Ref** +14, **Will** +16

**HP** 120; negative healing

**Immunities** death effects, disease, fear

**Weaknesses** positive 5

**Necrotic Decay** (divine, necromancy, negative) As urdefhan warrior (Bestiary 2 272), but DC 24 and 7d6 negative damage.

---

Speed 25 feet

**Melee** shauth lash +18 (deadly <d8>, trip); **Damage** 2d8+7 slashing plus Grab and Wicked Bite plus Grab, Wicked Bite

**Melee** jaws +17; **Damage** 2d6+7 plus Wicked Bite plus 2d6+7, Wicked Bite

**Innate divine Spells** (DC 22), attack +14

  **1st** feather fall, ray of enfeeblement

  **2nd** death knell

  **3rd** paralyze

**Constrict** ◆ 1d8+7 slashing, DC 25

**Shauth Seize** The lasher gains the Grab ability with a shauth lash and can use their Wicked Bite through magical shauth weapon Strikes.

**Wicked Bite** ◆ As urdefhan warrior (Bestiary 2 272), but DC 25 and the urdefhan regains 10 Hit Points when they Drink Blood if the target fails its saving throw (20 Hit Points on a critical failure).
