# Viper
*Creature -1*

n, tiny, animal

---

**Perception** +5; low-light vision, scent 30 feet (imprecise)

**Skills** Acrobatics +5, Athletics +1, Stealth +5, Survival +3

**STR** -3, **DEX** +4, **CON** +0, **INT** -4, **WIS** +1, **CHA** -2

---

**AC** 15

**Fort** +2, **Ref** +7, **Will** +5

**HP** 8

**Slink** ◈ **Trigger** A creature ends its movement adjacent to the viper or within the viper's space.; The viper Strides, Climbs, or Swims up to 10 feet (or up to the relevant Speed, if that Speed is less than 10 feet). It must end its movement in a location that isn't within 5 feet of a foe. This movement doesn't trigger reactions.

---

Speed 20 feet, swim 20 feet, climb 20 feet

**Melee** ◆ fangs +8 (agile, finesse); **Damage** 1d8-3 piercing plus viper venom plus viper venom

**Viper Venom** (poison)
