# Water Mephit
*Creature 1*

n, small, aquatic, elemental, water

---

**Perception** +3; darkvision

**Languages** aquan

**Skills** Athletics +6, Stealth +6

**STR** +1, **DEX** +3, **CON** +1, **INT** -2, **WIS** +0, **CHA** +0

---

**AC** 16

**Fort** +7, **Ref** +11, **Will** +4

**HP** 20; fast healing 2 (while underwater)

**Immunities** bleed, poison, paralyzed, sleep

---

Speed 20 feet, fly 25 feet, swim 25 feet

**Melee** ◆ claw +8 (finesse); **Damage** 1d6+1 slashing

**Breath Weapon** ◆◆ (acid, arcane) The water mephit breathes acid in a 15-foot cone that deals 2d6 acid damage to each creature within the area (DC 17 basic Reflex save). The water mephit can't use Breath Weapon again for 1d4 rounds.

**Drench** ◆ (abjuration, arcane, water) The water mephit puts out all fires in a 5-foot emanation. The mephit extinguishes all non-magical fires automatically and attempts to counteracting magical fires (+7 counteract modifier).
