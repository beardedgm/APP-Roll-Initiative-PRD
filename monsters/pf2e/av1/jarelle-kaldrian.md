# Jarelle Kaldrian
*Creature 5*

unique, le, medium, ghost, incorporeal, spirit, undead

---

**Perception** +12

**Languages** aklo, common, necril, undercommon

**STR** -5, **DEX** +5, **CON** +0, **INT** +2, **WIS** +3, **CHA** +0

**Site Bound** (area C13)

---

**AC** 21

**Fort** +9, **Ref** +14, **Will** +12

**HP** 48; negative healing, rejuvenation

**Immunities** death

**Rejuvenation** (divine, necromancy) Removing this room's tables and tools allows Jarelle to move on to the afterlife.

---

Speed fly 25 feet

**Melee** ghostly hand +14 (agile, finesse, magical); **Damage** 3d8 negative

**Malevolent Possession** ◆◆ (divine, incapacitation, mental, necromancy, possession) Jarelle attempts to possess an adjacent corporeal creature. This has the same effect as the possession spell (with a spell DC of 22), except since Jarelle doesn't have a physical body, she is unaffected by that restriction of the spell.

**Poisoned Breath** ◆◆ (divine, necromancy, poison) Jarelle exhales a 15-foot cone of toxic mist that deals 5d6 poison damage (DC 22 basic Fortitude save; on a critical failure, the target is also enfeebled|CRB|enfeebled 1 for 24 hours). She then can't use Poisoned Breath again for 1d4 rounds.
