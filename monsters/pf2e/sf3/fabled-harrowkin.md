# Fabled Harrowkin
*Creature 17*

rare, n, medium, construct

---

**Perception** +29; darkvision

**Languages** common

**Skills** Acrobatics +30, Athletics +32, Diplomacy +32, Occultism +29

**STR** +7, **DEX** +5, **CON** +7, **INT** +2, **WIS** +4, **CHA** +9

**Items** +2 greater striking greatsword, harrow deck (53 cards; function as darts when thrown by a harrowkin)

**Harrowkin Suit** any

**Read the Cards** ◇

---

**AC** 40

**Fort** +32, **Ref** +30, **Will** +27

**HP** 315

**Immunities** as {@creature simple harrowkin|SF3}

**Defensive Suit**

**Twist Fate** ◈ (divination, fortune, occult) **Trigger** A creature critically hits the harrowkin; **Frequency** {"number":1,"unit":"minute"}; The harrowkin manipulates fortune in their favor. The triggering attack becomes a regular hit.

---

Speed 25 feet, fly 40 feet

**Melee** ◆ greatsword +32 (magical, versatile <P>); **Damage** 3d12+15 slashing

**Ranged** ◆ harrow card +30 (agile, magical, range increment <20 feet>, thrown); **Damage** 3d6+15 piercing

**Innate occult Spells** (DC 35), attack +27

  **Cantrips (8th)** daze, detect magic, ghost sound, telekinetic projectile

  **1st** ill omen (at will)

  **2nd** augury

  **4th** confusion

  **5th** dimension door

  **7th** visions of danger

  **8th** uncontrollable dance
