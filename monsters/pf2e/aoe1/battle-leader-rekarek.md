# Battle Leader Rekarek
*Creature 2*

unique, le, small, humanoid, kobold

---

**Perception** +10; darkvision

**Languages** draconic

**Skills** Athletics +9, Intimidation +7, Stealth +6

**STR** +3, **DEX** +2, **CON** +4, **INT** -1, **WIS** +1, **CHA** +1

---

**AC** 17

**Fort** +12, **Ref** +8, **Will** +5

**HP** 40

---

Speed 20 feet

**Melee** ◆ greatpick +12 (fatal <d12>); **Damage** 1d10+3 piercing

**Melee** ◆ longspear +9 (reach); **Damage** 1d8+3 piercing

**Ranged** ◆ sling +10 (propulsive, range increment <50 feet>, reload <1>); **Damage** 1d6+1 bludgeoning

**Haphazard Hack** ◆◆ **Requirements** Rekarek is adjacent to two or more enemies; Rekarek makes three melee weapon Strikes, two of which must be against different enemies. These attacks count toward her multiple attack penalty, but only after all three Strikes have been made. After taking this activity, Rekarek is flat-footed until the beginning of her next turn.
