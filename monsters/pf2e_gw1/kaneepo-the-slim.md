# Kaneepo the Slim
*Creature 4*

unique, ce, medium, fey

---

**Perception** +14

**Languages** common, sylvan

**STR** +3, **DEX** +5, **CON** +2, **INT** +2, **WIS** +3, **CHA** +4

**Items** Shadewither Key (page 79)

---

**AC** 20

**Fort** +9, **Ref** +14, **Will** +14

**HP** 80

**Immunities** fear

**Weaknesses** cold iron 5

**Phasic Defenses** ◈ **Trigger** Kaneepo would take damage from an attack; **Requirements** Kaneepo hasn't Slimstepped since the beginning of their last turn; Kaneepo Slimsteps, taking no damage if their new square would move them out of the range or area of the attack.

---

Speed 30 feet

**Melee** claw +14; **Damage** 1d6+1 slashing plus shadowfeed

**Innate primal Spells** (DC 21)

**Create Breach** Once per day, Kaneepo can tear open the fabric between planes, creating a portal between the Thinlands and a point anywhere in Sevenarches. This portal lasts for up to 1 hour, but Kaneepo can close it at will.

**Shadowfeed** (fear, emotion, mental) Whenever Kaneepo hits a creature with an attack, Kaneepo's flayed strips of flesh wrap around the target's shadow, causing the victim's body to wither. The struck creature must attempt a DC 21 Fortitude saving throw. **Success** 
**Failure** 
**Critical Failure** 

**Shadow Pull** ◆◆ All creatures in a 60-foot line are affected by Kaneepo's shadowfeed ability.

**Slimstep** ◆ (conjuration, primal, shadow, teleportation) **Frequency** {"number":1,"unit":"round"}; **Requirements** Kaneepo is in dim light or darkness; Kaneepo instantly transports themself from one shadow to another. Kaneepo teleports themself and any items they're holding to a clear space within 30 feet that's in dim light or darkness. If this would bring any other creature with Kaneepo, the ability is disrupted.
