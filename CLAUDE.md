...
- Auth/webhook routes use `rateLimitByIP()` from `rateLimitGeneral.js` (DB-backed, reuses the 15-min TTL window). High-traffic public endpoints use the in-memory `rateLimitMemory()` (`rateLimitMemory.js`) instead — a DB-per-request limiter would add load to the hottest endpoints. In-memory counters are per-process (per-instance on a multi-instance deploy); acceptable as a flood cap. In-memory keys: `catalog-monsters` and `catalog-spells` (2000/15 min per IP each), `shared-encounter` (5000/15 min), `health` (600/15 min).
- **In-memory limiters must be path-scoped** (`router.use('/api/monsters', …)`), never pathless: catalog routers are mounted at the app root, so a pathless `router.use` counts *every* request in the app — including the SPA shell and static assets — against the cap and can 429 the whole site.
- `LoginAttempt` rows carry a `kind` field: `'auth'` (real login attempts, default) vs `'action'` (synthetic rows written by `rateLimitByIP`). The auth limiter (`rateLimitAuth.js`) counts **only** `kind: 'auth'` rows within an explicit `at >= windowStart` predicate — otherwise ordinary browsing (health probes, general-limiter actions) would lock out login for the IP, and TTL-sweeper lag would over-block past the advertised window.
- Thresholds: login 10/15 min, registration 5/15 min, password reset 5/15 min, change-password & delete-account 5/15 min (both are password-guess oracles), sitemap 30/15 min. The shared player-view endpoint (`/api/shared/:code`) is 5000/15 min in-memory: the view polls every 2s (~450 req/window per viewer) and a whole table behind one NAT shares an IP, so the cap must clear ~10 viewers while still stopping floods.
...
4. **Auto-seed on deploy (off the build, self-cleaning).** Content markdown in `monsters/` and `spells/` is seeded via Render's `preDeployCommand` (not `buildCommand`), so a failed seed fails the deploy instead of corrupting a live catalog mid-serve. Seeding validates each record (no silent `ac:10`/`crNumeric:0`/`hp:0` defaults — invalid files are reported and skipped unless `--fail-on-invalid`) and **reconciles stale records**: seeded DB docs whose source `.md` no longer exists are deleted, scoped to `{ isCustom: false }` for monsters (whole collection for spells), guarded by a 10% mass-delete abort and a `--dry-run` mode. The on-disk slug set is built from filenames, so a parse failure never deletes a doc. Never run the delete reconciliation from `app.js` boot (multi-instance race). Run `npm run seed:monsters -- --dry-run` before any production run that follows large content deletions. For local development, run the seed commands manually after markdown changes.
...
- **Seed dir casing**: The seed scripts must read the lowercase `monsters/` and `spells/` dirs (the tracked names). A capitalized path (e.g. `Monsters/`) resolves on case-insensitive Windows but finds **nothing** on Render's case-sensitive Linux FS — a silently empty scan, which is dangerous now that the seed deletes stale records (the 10% guardrail aborts such a run rather than wiping the catalog).
- **Seed `--dry-run` must reach the script**: the root `seed:monsters`/`seed:spells` scripts invoke `node server/scripts/*.js` directly (NOT `cd server && npm run …`). A nested `npm run` swallows `--dry-run` as *npm's own* flag, so `npm run seed:monsters -- --dry-run` would silently run a **real** seed. Confirm the `[DRY RUN] No writes will occur.` banner before trusting any dry run, and never route a seed through a nested `npm run`.
...
- **Coordinated major dep upgrades**: bump companion packages together or `npm install` fails with `ERESOLVE`. vite ↔ `@vitejs/plugin-react`; eslint ↔ `@eslint/js` ↔ `eslint-plugin-react-hooks` ↔ `eslint-plugin-react-refresh`. Dependabot splits these into separate PRs that each fail alone — combine them in one PR (and close the singletons).
- **Node 20 is a hard target** (`.node-version`, `engines.node`, CI, Render): check a major bump's `engines.node` before taking it. `concurrently@10` requires Node 22, so its major updates are ignored in `.github/dependabot.yml`. Dependabot minor/patch updates are grouped per directory there.
- **Transitive CVEs**: CI's audit gate is `npm audit --omit=dev --audit-level=critical`, so high/moderate transitive advisories don't fail CI and `npm audit fix` often can't bump them in-range. Patch with npm `overrides` (client pins `protobufjs` via posthog-js → OpenTelemetry).
- **eslint-plugin-react-hooks (v7.1.1+, eslint 10)**: don't call `setState` synchronously in an effect body — derive initial state instead (e.g. `useState(token ? 'loading' : 'invalid')`). A `useRef` seeded from a hook value can't be mutated; init `useRef(null)`, assign `.current` in a no-dep `useEffect`, and declare the ref *before* any callback that reads it.
- **Cross-system validator ranges (5e + PF2e)**: any Zod field validating a custom monster (`customMonsterSchema` in `server/validators/userData.js`) must accept the *union* of both systems' value semantics — PF2e `abilities` are signed **modifiers** (default `0`, often negative), not 1–30 scores; `ac` and `initMod`/Perception run higher than 5e. Match the form's own input bounds, not 5e rules. A too-narrow range here is silent: `PUT /api/user-data` now validates per item and *drops* (skip+`warn`-log) invalid records rather than 400-ing the batch, so a bad range loses that monster from cloud sync without any error surfacing.
